...
ImageProcess::ImageProcess(VideoMemory* const pVideoMemory, InternalMemory* const pInternalMemory, DataRead* const pDataRead)
	:	m_pVideoMemory(pVideoMemory), m_pInternalMemory(pInternalMemory), m_pDataRead(pDataRead)
{
...
uint8_t ImageProcess::getBufferData(uint8_t horizontal_addr_vram, uint8_t vertical_addr_vram) 
{
	...
	// this is carried out by reading dataArray information into the internal memory buffer
		...
		if(temp_page_number != m_page_number)
		{
			...
			m_pDataRead->readInternalStorage(m_page_number*BUFFER_SIZE);
		...
		return m_pInternalMemory->readMemoryLocation(m_page_address);
...
void ImageProcess::readImageConfigData()
{
	...
	m_pDataRead->readInternalStorage(0);

	
	// read offset 22h,23h,24h,25h in buffer into m_image_size
	m_raw_image_size |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x25) << 24); 
	m_raw_image_size |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x24) << 16); 
	m_raw_image_size |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x23) << 8);
	m_raw_image_size |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x22));
	
	
	// read offset Ah,Bh,Ch,Dh in buffer into m_image_raw_data_offset
	m_image_raw_data_offset |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x0D) << 24);
	m_image_raw_data_offset |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x0C) << 16);
	m_image_raw_data_offset |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x0B) << 8);
	m_image_raw_data_offset |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x0A));
	

	// read offset 12h, 13h, 14h, 15h into m_image_width
	m_image_width |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x15) << 24);
	m_image_width |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x14) << 16);
	m_image_width |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x13) << 8);
	m_image_width |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x12));
	
	
	// read offset 16h, 17h, 18h, 19h into m_image_height
	m_image_height |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x19) << 24);
	m_image_height |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x18) << 16);
	m_image_height |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x17) << 8);
	m_image_height |= ((uint32_t)m_pInternalMemory->readMemoryLocation(0x16));
